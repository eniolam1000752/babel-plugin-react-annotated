...
const _useStateTemplate = function (initValueNode, idText) {
  ...
  const objectConstruct = types.objectExpression([types.objectProperty(types.identifier(idText), initValueNode)]);
  const leftExpression = types.arrayPattern([types.objectPattern([types.objectProperty(types.identifier(idText), types.identifier(idText), false, true)]), types.identifier(`${setStatePrefix}${idText}`)]);
  const RightExpression = types.callExpression(types.memberExpression(types.identifier("React"), types.identifier("useState")), [objectConstruct]);
...
const _nestedUpdateExpTemplate = function (dummyVar, nestNode, higerStateName, trueVar, LONA, path) {
  ...
  const buildASTNode = !isLeftMemberExp ? template(`${setStatePrefix}${trueVar.name}(STATE_NAME => {STATE_CLONE; MEMBER_STATE_NAME = HIGH_STATE_NAME; NEST_STATE_NODE ;return STATE_NAME})`) : template(`${setStatePrefix}${getMemberExpStateName(trueVar)}(STATE_NAME => {STATE_CLONE;LEFT_EXP  = HIGH_STATE_NAME; NEST_STATE_NODE ;return STATE_NAME})`);
  ...
  const temp2 = !isIdentifierReactState(path, nextAssignLeft) ? leftNodeOfNextAssignment : temp;
  let out = {
  ...
  return buildASTNode(!isLeftMemberExp ? { ...out,
...
const assignmentTemplate = function (leftNode, rightNode, LONA, node, path) {
  ...
  const varInExp = LONA ? leftNodeOfNextAssignment.name || getMemberExpStateName(leftNodeOfNextAssignment) : null;
  ...
  if (LONA) {
    if (LONA && isNextLeftMemberExp && isIdentifierReactState(path, varInExp)) {
      ...
      rightNode = !isIdentifierReactState(path, varInExp) ? leftNodeOfNextAssignment.name : memberExpNode(leftNodeOfNextAssignment.name, rightNode);
    }
  }
...
const _stateWithReturnTemplate = function (equivNodeRight, name, nameExp) {
  const setStateNode = template(`${setStatePrefix}${name}( ARGS => { NODE_EQUIV; return ARGS;  })`);
  const declare1 = types.variableDeclaration("let", [types.variableDeclarator(types.identifier(DUMMY_NAME), nameExp.argument)]);
...
const check__PrefixSyntax = function (node, path) {
  for (let item of node.declarations) {
    ...
    if (new RegExp(`^${statePrefix}\\w+$`).test(varName)) {
      throw path.buildCodeFrameError(`Error in build: non anotated variable should not have '${statePrefix}' prefix for variable: '${varName}'`);
    }
  }
};
...
const expressionVisistor = {
  AssignmentExpression(path) {
    ...
    let node = path.node;
    this.varName = stateNames.indexOf(node.left.name || getMemberExpStateName(node.left)) !== -1 ? node.left.name || getMemberExpStateName(node.left) : null;
    ...
    if (this.varName && !types.isAssignmentExpression(node.right) && isIdentifierReactState(path, this.varName)) {
      ...
      if (types.isUpdateExpression(node.right) && isIdentifierReactState(path, node.right.argument)) {
    ...
    if (types.isAssignmentExpression(node.right) && !types.isAssignmentExpression(path.parent)) {
      ...
      let reculsive = assignNode => {
        if (types.isAssignmentExpression(assignNode.right) && isIdentifierReactState(path, assignNode.left.name || getMemberExpStateName(assignNode.left)) ? true : assignNode.right.operator === "=") {
          resultAssignNode = isIdentifierReactState(path, assignNode.left.name || getMemberExpStateName(assignNode.left)) ? _nestedUpdateExpTemplate(higherState || `_RN_${genVar()}`, resultAssignNode, higherState = `_RN_${genVar()}`, assignNode.left, assignNode.right.left, path) : !resultAssignNode ? assignmentTemplate(assignNode.left, higherState = `_RN_${genVar()}`, assignNode.right.left, null, path) : !resultAssignNode.length ? [assignmentTemplate(assignNode.left, higherState, assignNode.right.left, null, path), resultAssignNode] : [assignmentTemplate(assignNode.left, higherState, assignNode.right.left, null, path), ...resultAssignNode];
          ...
          if (resultAssignNode) {
            ...
            if (types.isUpdateExpression(assignNode.right) && isIdentifierReactState(path, assignNode.right.argument.name || getMemberExpStateName(assignNode.right.argument))) {
              ...
              if (types.isMemberExpression(assignNode.right.argument)) {
                replaceStateWithDummyInMemberExp(assignNode.right.argument, DUMMY_NAME, memberExpNode(name, DUMMY_NAME));
              } else if (types.isIdentifier(assignNode.right.argument)) {
                assignNode.right.argument.name = DUMMY_NAME;
              }

              assignNode.right = _stateWithReturnTemplate(types.cloneNode(assignNode.right), name, nodeClone);
            }

            resultAssignNode = isIdentifierReactState(path, varInExp) ? _updateExpressionTemplate2(higherState, assignNode, resultAssignNode, leftNode) : !resultAssignNode.length ? [assignmentTemplate(higherStateNode, assignNode.right, null, null, path), assignmentTemplate(leftNode, higherState, null, assignNode, path), resultAssignNode] : [assignmentTemplate(higherStateNode, assignNode.right, null, null, path), assignmentTemplate(leftNode, higherState, null, assignNode, path), ...resultAssignNode];
          }
        }
      };
    ...
    if (types.isMemberExpression(node.left) && !types.isAssignmentExpression(path.parent) && !types.isAssignmentExpression(node.right)) {
      this.varName = isIdentifierReactState(path, getMemberExpStateName(node.left)) ? getMemberExpStateName(node.left) : null;
      path.traverse({
        Identifier(path) {
          if (path.node.name === this.varName) {
  ...
  UpdateExpression(path) {
    ...
    this.varName = isIdentifierReactState(path, args.name || getMemberExpStateName(args)) ? args.name || getMemberExpStateName(args) : null;
...
function initExpression(path, transformedPath, typeExp) {
  ...
  const immidateTopComment = expStatement.leadingComments ? expStatement.leadingComments[expStatement.leadingComments.length - 1] : null;
...
function putNodeInUseEffect(parentBlockExp, childNode) {
  ...
  parentBlockExp.body = parentBlockExp.body.reduce((cum, item) => types.isReturnStatement(item) ? [...cum, node, item] : [...cum, item], []);
  useEffectNode = node;
}
...
const replaceStateWithDummyInMemberExp = (memberExp, dummyVar, overrideVarNode) => {
  let func = exp => {
...
const isParentReactElement = function (nodePath) {
  ...
  if (types.isFunctionDeclaration(parentNode) || types.isArrowFunctionExpression(parentNode) || types.isFunctionExpression(parentNode)) {
    if (types.isFunctionDeclaration(parentNode)) {
      return /^_?[A-Z]/.test(parentNode.id ? parentNode.id.name : "") || /^use/.test(parentNode.id ? parentNode.id.name : "");
    }
